import osmnx as ox
from osmnx import downloader, graph
...
def create_branch_graph(response_json_branch,
                        retain_all=False, bidirectional=True):
    """
    ...
    response_json_branch : dict
        dicts of JSON response for a route branch sfrom the Overpass API
    retain_all : bool
...
def query_branch_nodes_paths(response_json_branch):
    ...
    # get path ids
    path_ids = [m['ref'] if m['type'] ==
                'way'else None for m in response_json_branch['members']]
...
def query_public_transport_box(north, south, east, west):
    ...
    response_json_branches = ox.downloader.overpass_request(
        data={'data': query_str})
    # make sure we got data back from the server request(s)
    if 'elements' not in response_json_branches:  # pragma: no cover
        raise EmptyOverpassResponse(
            "There are no data elements in the response JSON")
    else:
